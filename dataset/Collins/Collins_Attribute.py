...
folder_path = "collins_subnetwork"
for h in os.listdir(folder_path):
    node_file = h.split('.')[0]
    str1 = f"{folder_path}/{h}"
    str2 = f"collins_attr/Attribute_{node_file}.txt"
    file1 = open(str1)
    node = []
    for j in file1:
        temp1 = j.split()[0]
    ...
    go = []
    file = open("collins_go_information.txt")
    file4 = open("collins_go_information_temp.txt", 'w')
    for i in file:
    ...
    file = open("collins_go_information_temp.txt")
    gov = []
    for i in file:
        node_name = i.split(' ', 1)[0]
        node_go = i.split(' ', 1)[1].rstrip('\n').rstrip(' ')
